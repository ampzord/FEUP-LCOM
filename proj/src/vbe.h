...
typedef struct {
	...
	uint16_t	vbe_version;			//0200h - VBE version
	uint32_t	oem_string_ptr;			//Pointer to OEM String
	uint8_t		capabilities[4];		//Capabilities of graphics controller - (4 dup ?)
	uint16_t	video_mode_ptr;			//Pointer to VideoModeList
	...
	uint8_t		reserved[222];			//Reserved for VBE implementation scratch area - (222 dup ?)
	uint8_t		oem_data[256];			//Data Area for OEM Strings - (256 dup ?)

} __attribute__ ((packed)) vbe_information_block_t;

/** @} end of vbe_information_block_t*/
...
uint16_t* vbe_get_information_block(vbe_information_block_t *controller_info);
